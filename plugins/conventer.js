    ...
    lusifar.LUSIFARCMD({pattern: 'stvid$', desc: Lang.ANİM_STİCK, fromMe: true}, (async (message, match) => {
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                const saving = data.pipe(fs.createWriteStream('/root/Lusifarwhatsappbot/stweb.mp4'))
                saving.on("finish", async () => {
                    await message.client.sendMessage(mid, fs.readFileSync('/root/Lusifarwhatsappbot/stweb.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: Config.AFN, quoted: message.data })
                    if (fs.existsSync(savedFilename)) fs.unlinkSync(savedFilename)
                    if (fs.existsSync('/root/Lusifarwhatsappbot/stweb.mp4')) fs.unlinkSync('/root/Lusifarwhatsappbot/stweb.mp4')
                })
            })
        })
    }));
